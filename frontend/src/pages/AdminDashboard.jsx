      ...
      <div className="bg-gradient-to-r from-blue-600 to-cyan-600 p-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-blue-100 text-sm">Hierarchy: Districts → Villages → Transformers → Houses</p>
          </div>
        </div>
      </div>
        ...
        <div className="p-4">
          ...
          <div className="space-y-2 max-h-[300px] overflow-y-auto">
            {selectedTransformer?.houses?.map(house => (
              <div
                key={house.id}
                className="p-3 rounded-lg bg-gray-50 border-2 border-transparent hover:bg-purple-50"
              >
        ...
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-purple-200 text-sm">Electricity Distribution Monitoring</p>
            </div>
          </div>
          <div className="flex items-center gap-4">
            <span className="text-purple-200 text-sm">Updated: {formatTime(lastUpdated)}</span>
            <button onClick={handleLogout} className="flex items-center gap-2 px-4 py-2 bg-red-500/20 hover:bg-red-500/30 text-red-200 rounded-lg">
        ...
        <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-purple-200 text-sm">Real-time monitoring across {summary.total_houses} houses</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-sm ${
                  connectionStatus === 'connected' ? 'bg-green-500/20 text-green-100' : 'bg-red-500/20 text-red-100'
                }`}>
                  <span className={`w-2 h-2 rounded-full ${connectionStatus === 'connected' ? 'bg-green-400 animate-pulse' : 'bg-red-400'}`}></span>
                  {connectionStatus === 'connected' ? 'Connected' : 'Disconnected'}
                </div>
                <button onClick={fetchData} disabled={loading} className="p-2.5 bg-white/20 hover:bg-white/30 rounded-xl">
          ...
          <div className="grid grid-cols-4 lg:grid-cols-8 gap-3 p-4 bg-white border-t">
            ...
            <div className="text-center p-2 bg-purple-50 rounded-lg">
              <p className="text-xl font-bold text-purple-600">{summary.total_power_kw.toFixed(0)}</p>
              <p className="text-xs text-purple-600">kW</p>
            </div>
          </div>
        </div>
